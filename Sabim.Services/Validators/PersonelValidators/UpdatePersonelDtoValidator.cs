...
namespace Sabim.Services.Validators.PersonelValidators
{
    public class UpdatePersonelDtoValidator : AbstractValidator<UpdatePersonelDto>
    {
        ...
        public UpdatePersonelDtoValidator(IServiceManager manager)
        {
            _manager = manager;
            RuleFor(x => x.PersonelID).NotEmpty().WithMessage("Personel ID alanı boş bırakılamaz.");
            RuleFor(x => x.Ad)
            ...
            RuleFor(x => x.SicilNumarasi)
                      ...
                      .Must((dto, sicilNumarasi) => BeUniqueSicilNumarasi(sicilNumarasi, dto.PersonelID)).WithMessage("Aynı sicil numarası zaten kayıtlı!");
            ...
            RuleFor(x => x.KimlikNo)
                ...
                .Must((dto, kimlikNo) => BeUniqueKimlikNo(kimlikNo, dto.PersonelID)).WithMessage("Aynı kimlik numarası zaten kayıtlı!")
        ...
        private bool BeUniqueSicilNumarasi(int sicilNumarasi, short? excludeId)
        {
            return !_manager.PersonelService.TIsAny(x => x.SicilNumarasi == sicilNumarasi, excludeId);
        }
        private bool BeUniqueKimlikNo(string kimlikNo, short? excludeId)
        {
            return !_manager.PersonelService.TIsAny(x => x.KimlikNo == kimlikNo, excludeId);
        }
    }
}
