...
{
    public class UpdateKisimDtoValidator : AbstractValidator<UpdateKisimDto>
    {
        private readonly IServiceManager _manager;

        public UpdateKisimDtoValidator(IServiceManager manager)
        {
            _manager = manager;
            RuleFor(x => x.KisimAdi)
                       ...
                       .Must((dto, kisimAdi) => BeUniqueName(kisimAdi, dto.KisimID, dto.BirimId)).WithMessage("Aynı birimde bu kısım adı zaten kayıtlı!");
        ...
        private bool BeUniqueName(string kisimAdi, short birimId, short? excludeId)
        {
            return !_manager.KurumService.TIsKurumExists(kisimAdi, birimId, excludeId);
        }
    }
}
